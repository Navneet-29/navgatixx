...
export const DriverRouteMap: React.FC<DriverRouteMapProps> = ({
    ...
    const fetchStage1 = async () => {
      try {
        ...
        if (data.routes && data.routes.length > 0) {
          ...
          if (!isGoodsLoaded) {
            const firstStep = r.legs?.[0]?.steps?.[0]?.maneuver?.instruction || r.legs?.[0]?.steps?.[0]?.name || 'Head towards customer pickup';
            setRouteSummaryText({
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm bg-white mt-4">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-bold text-slate-100 truncate mt-0.5">
                {!isGoodsLoaded
                  ? `To: ${pickupAddress || "Customer Pickup"}`
                  : `To: ${dropAddress || "Destination Drop"}`}
              </p>
            </div>
          </div>
            ...
            <Marker position={[driverLat, driverLng]} icon={carIcon}>
              <Popup>
                <div className="font-bold text-xs">
                  <p className="text-indigo-600 uppercase font-black">🚗 Your Live Location</p>
